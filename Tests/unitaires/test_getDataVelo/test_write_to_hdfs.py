...
#  Cas standard : vérifier que df.write.mode("append").parquet(path) est appelé avec les valeurs par défaut.
...
# Cas alternatif : le mode est passé explicitement en paramètre (e.g. "overwrite").
def test_write_to_hdfs_calls_parquet_custom_mode():
    ...
    write_to_hdfs(mock_df, "hdfs://custom/path", mode="overwrite")
